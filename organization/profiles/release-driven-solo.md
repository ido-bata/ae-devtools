...
ae-devtools の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues / Pull Requests を current planning / delivery surface とし、未採用の Linear をこの reconciliation だけで canonical 化しない。
- GitHub Issues が durable implementation / dependency facts の canonical authority
- planning surface は implementation SoT を置換しない
...
- 初期設計段階の未確定事項を勝手に固定しない既存 `docs/vision.md` / AGENTS contract を保持する。
- VSCode Extension / MCP Server / AE bridge / shared core の責務境界を project-specific architecture authority とする。
- After Effects host verification は source-only validation と分離し、current candidate identity に bind する。

## Upstream source

`rebuildup/project-init@release-0-3-0` (`57fb4a2e5abe6f52e4fd9cb2cb88234496e47d4b`) を基準に reconcile する。
