...
| M4 | VSCode ExtendScript Debugger native addon (`esdcorelibinterface.node` / `ESCoreLib`) | 01, 04 | **Rejected (for v1)** | Platform coverage Windows + macOS x64 only; Adobe-CEP org archived; source not directly verifiable. Keep as a future escape hatch if M1+M3 prove insufficient on a particular host. |
...
- `adobe.com/content/dam/cc/en/think-tank/uxp-in-after-effects/uxp-in-after-effects.pdf` — Adobe UXP in AE whitepaper (URL exists; WebFetch returned 404 at research time — URL existence is the verified fact).
